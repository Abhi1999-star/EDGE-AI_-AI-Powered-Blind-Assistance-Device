...
while(True):
    ...
    for i, detection_list in enumerate(predictions):
        ...
        if len(detection_list) == 0: continue  # Skip if nothing detected

        if i >= len(labels):  # Prevent "index out of range" crash
            print("Detected unknown class ID:", i)
            continue
        ...
        for x, y, w, h, score in detection_list:
            ...
            print(f"x {center_x}\ty {center_y}\tscore {score:.6f}")
